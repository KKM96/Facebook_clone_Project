...
// Multer setup                                                              // urlencoded는 텍스트가 아닌 이미지, 동영상 등의 파일 데이터는 처리 x => multer 모듈 이용
const storage = multer.diskStorage({                                         // 저장 경로와 파일명을 처리하기 위해 diskStorage() 메서드 이용
    filename: (req, file, callback) => {                                               
        callback(null, Date.now() + file.originalname);
    }
});

const imageFilter = (req, file, callback) => {                              // 파일 확장자 확인 부분, 확장자가 이상하면 에러 메세지 콜백 함수 반환
...
// Cloudinary setup                                                         // 이미지 업로드 및 불러올 공간을 빌리기 위해 SaaS 서비스 cloudinary 사용
cloudinary.config({
...
// Middleware                                                              // 로그인하지 않은 사용자를 체크하는 미들웨어부분 
const isLoggedIn = (req, res, next) => {
    if (req.isAuthenticated()) {
        return next();
    }
    req.flash("error", "You need to be logged in to do that!");            // 로그인 하지 않았을 시에 flash로 오류 메시지 보내기
...
// User Routers 
router.post("/user/register", upload.single("image"), (req, res) => {     // 회원가입 라우터
    if (
        req.body.username &&
        req.body.firstname &&
        req.body.lastname &&
        ...
        let newUser = new User({                                         // req.body 객체의 ~들을 받아 newUser 객체에 넣기
            username: req.body.username,
            firstName: req.body.firstname,
            lastName: req.body.lastname
        });
        if (req.file) {                                                 // 처음 회원가입시 프로필 이미지를 받고 이를 multer를 통해 req.file에 설정
            cloudinary.uploader.upload(req.file.path, result => {
                newUser.profile = result.secure_url;
                return createUser(newUser, req.body.password, req, res);
            });
        } else {
            newUser.profile = process.env.DEFAULT_PROFILE_PIC;         // 프로필 사진이 없을땐 Default_profile_pic을 프로필 사진으로 지정
            return createUser(newUser, req.body.password, req, res);
        }
    }
});

function createUser(newUser, password, req, res) {                      // create user 함수 // 오류 or 완료 메시지 flash()
